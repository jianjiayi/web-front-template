...
const errorHandler = ({ showToast = true }) => (error) => {
  ...
  if (response.code !== successCode && showToast) {
    const message = response.msg || codeMessage[response.code] || '未知错误！';
    notification.error({
      message,
    });
  }
...
request.interceptors.request.use((url, { credentials = 'include', ...options }) => {
  const newOptions = {
    method: 'get',
    headers: {},
    ...options,
...
request.interceptors.response.use(async (response, { showToast = true }) => {
  ...
  if (response && response.status) {
    // 未登录跳转
    if (response.status === 401 && history.location.pathname !== PATH_USER_LOGIN) {
